...
def train_model(
    ...
    print("Loading data...")
    print("Data directory:", dir_path)  # Add this line to check the value of dir_path
...
def gen_parser():
    ...
    #epoch = options.epoch if options.epoch else 5 
...
def train(model, device, data_loader, criterion, optimizer, epoch, train_loss, train_acc, mse, scaler):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(data_loader):
        ...
        with autocast():  # Use autocast to cast operations to appropriate precision
            outputs = model(inputs)
            loss = criterion(outputs, targets)

        scaler.scale(loss).backward()  # Scale the loss value
        scaler.step(optimizer)  # Adjust the optimizer parameters
...
def main():
    ...
    data_dir, mod, epoch, pretrained_model_filename, test_model, batch_size = gen_parser()
    if data_dir is None:
        print("Error: Data directory not provided.")
        sys.exit(1)
    train_model(data_dir, mod, epoch, pretrained_model_filename, test_model, batch_size)
